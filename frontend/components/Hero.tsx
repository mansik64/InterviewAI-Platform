  ...
  return (
    <section className="w-full pt-24 pb-10 bg-white flex flex-col items-center">
      <div className="max-w-5xl w-full px-6 flex flex-col items-center text-center">
        ...
        <p className="max-w-2xl text-base md:text-lg text-slate-500 mb-6 leading-relaxed">
        ...
        <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-4 py-6 border-y border-slate-100 mb-8">
            {[['10k+', 'Interviews'], ['98%', 'Success Rate'], ['50+', 'Job Roles'], ['24/7', 'AI Support']].map(([val, label]) => (
              <div key={label}>
                  <h5 className="text-2xl font-black text-slate-900">{val}</h5>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <button onClick={() => handleAction("/resume")} className="px-10 py-3.5 bg-white text-slate-900 font-bold rounded-xl border-2 border-slate-200 hover:border-slate-900 transition-all text-sm">
            ANALYZE RESUME
          </button>
        </div>
      </div>
